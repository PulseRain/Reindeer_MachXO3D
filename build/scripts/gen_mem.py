...
def main():
    ...
    if (len(sys.argv) <= 1):
        print ("Usage: python gen_mem.py hex_file_name [bank index]")
        exit(1)
        
    intel_hex_file =  Intel_Hex(sys.argv[1])

    addr = 0;
      
    for record in intel_hex_file.data_record_list:

        new_addr = (record.address - 0x80000000) // 4
        
        delta = new_addr - addr
        for i in range(delta):
            if (len(sys.argv) <= 2):
                print ("0x%x %08x" % (addr*4, 0xdeadbeef))
            else:
                print ("%02x" % (0xbf))
            addr = addr + 1
            
        for i in range(len(record.data_list)//4):
           k =  record.data_list[0 + i*4] + (record.data_list[1 + i*4] << 8) + (record.data_list[2 + i*4] << 16) + (record.data_list[3 + i*4] << 24)
           
           if (len(sys.argv) <= 2):
               print ("0x%x %08x" % (addr*4, k))
           else:
               k = (k >> (8 * int(sys.argv[2]))) & 0xFF
               print ("%02x" % (k))
